effective_dis = readtable('final_results.xlsx','Sheet', 'effective_dis','PreserveVariableNames',true);
infected_time = readtable('final_results.xlsx','Sheet', 'infected_time','PreserveVariableNames',true);
node_type = table2cell(readtable('final_results.xlsx','Sheet', 'node_type','PreserveVariableNames',false));
iso_code = readtable('final_results.xlsx','Sheet', 'iso_code','PreserveVariableNames',true);
detailed_countries = table2cell(readtable('final_results.xlsx','Sheet', 'detailed_countries','PreserveVariableNames',true));
detailed_countries_index = table2array(readtable('final_results.xlsx','Sheet', 'detailed_countries_index','PreserveVariableNames',true));
...
for row=1:2
    for col=1:2
        ...
        scatter(infected_time_p(node_type_china),effective_dis_p(node_type_china), 130,'MarkerEdgeColor','black','MarkerFaceColor',china_color)
        hold on
...
%         for j=1:length(detailed_countries)
%             detailed_country_index = int8(detailed_countries_index(j));
%             detailed_country_name = string(detailed_countries(j));
%             text(infected_time_p(detailed_country_index+1), ...
%                  effective_dis_p(detailed_country_index+1), ...
%                 detailed_country_name, 'FontSize',font_size,'FontWeight','bold');
%         hold on
%         end
