...
def define_epidemic_paras(delta_t_):
    ...
    DE_ = 1.2 / delta_t_  # incubation period
    theta_ = 1 / 3.85
    beta_ = R0_ / DI_
    lambda_ = beta_ - 1 / DI_
    gamma_ = 0.5772156649
    return beta_, DI_, DE_, lambda_, gamma_, theta_
...
def SEIR_trans(N_, S_, E_, I_, R_, omega_, beta_, incubation_period_, infectious_period_, theta_, e=1e-10):
    SE_trans = S_ * (I_ + theta_ * E_) / N_ * beta_ * numpy_sigma((E_+I_) / N_ / e)
    EI_trans = E_ / incubation_period_
    IR_trans = I_ / infectious_period_

    d_S = - SE_trans + cross_minus_new(omega_, S_)
    d_E = SE_trans - EI_trans + cross_minus_new(omega_, E_)
    d_I = EI_trans - IR_trans + cross_minus_new(omega_, I_)
    ...
    E_ = np.minimum(Popu, np.maximum(np.zeros_like(Popu), E_))
...
beta, DI, DE, lambda_epi, gamma, theta = define_epidemic_paras(delta_t)
# ----------------------------------------------------
...
for top_percent in top_percent_list:
    ...
    for t in range(1, T + 1):
        S, E, I, R = SEIR_trans(Popu, S, E, I, R, omega, beta, DE, DI, theta)
        for xx in np.where((I >= 1) | (E >= 1))[0]:
...
# ----------- saving results -------------------------------------
removed_nbrs_add_length = {
    p: removed_nbrs[p] + ['X'] * (len(removed_nbrs[str(top_percent_list[-1])]) - len(removed_nbrs[p]))
    for p in removed_nbrs}

df = pd.DataFrame(removed_nbrs_add_length)
df.to_excel('results/seir_final_results.xlsx', sheet_name='removed_nbrs', index=False)

df = pd.DataFrame(infected_time_all)
with pd.ExcelWriter('results/seir_final_results.xlsx', engine="openpyxl", mode='a') as writer:
